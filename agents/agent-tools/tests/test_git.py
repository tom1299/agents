...
class TestGit:
    ...
    def test_clone_repository(self) -> None:
        ...
        assert os.path.exists(git_folder_path), f"Expected {git_folder_path} to exist after cloning."

        readme_path = f"{target_folder}/README.md"
        assert os.path.exists(readme_path), f"Expected {readme_path} to exist after cloning the."
...
class TestGitFileHistory:
    ...
    def test_get_file_commit_history(self) -> None:
        ...
        assert len(commit_lines) == 10, f"Expected 10 commits in the history, but found {len(commit_lines)}."
    ...
    def test_get_commit_details_from_history(self):
        ...
        assert len(commit_details_list) == 10, f"Expected 10 commit details, but found {len(commit_details_list)}."
    ...
    def test_create_change_from_commit_detail(self):
        ...
        for commit_detail in commit_details_list:
            ...
            if change.commit_hash == "854aaf863c572486e8998060294c4d858dc74101":
                ...
                assert expected_date == expected_date_str, f"Expected date to be {expected_date_str}, but got {expected_date}."

                assert "Fix ordering of Service and Pod in Port definitions example" in change.subject
            elif change.commit_hash == "434ae53916eca3e50394ffb47320b0908d41fe3f":
                assert change.commit_message.startswith("Switches the existing")
                assert all(s in change.commit_message for s in ["feature-state shortcode", "across the docs."])
